...
export const getProcessDetailField = (item, key) => {
  if (typeof item.processDetail === 'string') {
    item.processDetail = JSON.parse(item.processDetail)
    return item.processDetail[key]
  }
  return item.processDetail ? item.processDetail[key] : ''
}
